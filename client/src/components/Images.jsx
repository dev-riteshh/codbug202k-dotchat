    ...
    <>
      ...
      {images.length === 0  && (
            <div className="h-full flex justify-center items-center">
              <h1 className="text-[2rem]">🔎 Search Images here</h1>
            </div>
        )}
        ...
        <div id="images" className="grid gap-3 grid-cols-3">
          ...
          {images.map((image) => (
            <img className='rounded-md h-[300px] w-[500px]' src={image.link} alt={image.title} key={image.link} />
          ))}
        </div>
      </div>

      <div className="">
      <form onSubmit={SendMessage} className="flex p-2 px-4 bg-gray-700  rounded-lg items-center">
            <input
              value={input}
              onChange={(e) =>
